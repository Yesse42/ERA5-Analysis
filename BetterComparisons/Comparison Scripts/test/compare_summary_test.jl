...
for stat in basin_to_courses[basin]
    ...
    mediangroup = groupby(
        filter(
            x -> 1991 <= year(x.datetime) <= 2020,
            transform(comboed, :datetime => ByRow(shifted_month) => :shiftmonth),
        ),
        :shiftmonth,
    )
    mymedian(x) =
    ...
    group_shiftmonth = groupby(
        transform(comboed, :datetime => ByRow(shifted_month) => :shiftmonth),
        :shiftmonth,
    )
    #And now get the rmsd
    rmsd_data =
        combine(group_shiftmonth, fomcols => StatsBase.rmsd => :rmsd, nrow => :n_obs)

    #And push just the 3th time to the basin mean
    filter!(x -> x.shiftmonth == 3, rmsd_data)
    if isempty(rmsd_data)
        continue
    end
    insert!(
        outdict,
        stat,
        (rmsd = only(rmsd_data[:, :rmsd]), n_obs = only(rmsd_data[:, :n_obs])),
    )
