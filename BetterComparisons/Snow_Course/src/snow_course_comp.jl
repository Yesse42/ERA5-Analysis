...
for basin in ERA.basin_names
    ...
    for eratype in ERA.eratypes
        ...
        for id in courses
            ...
            analyzed_data = comparison_summary(data, ["$eratype", :snow_course_swe], :datetime; anom_stat="median").monthmeandata
            push!(used_courses, id)
            push!(course_data, analyzed_data)
        end

        basinmean = sort!(basin_aggregate(course_data, used_courses; timecol="month"), "month")
