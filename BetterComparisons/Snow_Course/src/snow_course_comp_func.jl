...
snow_course_from_basin = jldopen(joinpath(ERA.NRCSDATA, "cleansed", "Snow_Course_basin_to_id.jld2"))["basin_to_id"]

default_compare_args = (;load_data_func = load_snow_course,
...
default_omniplot_args = (;stat_swe_name = "snow_course_swe_fom_mean",
...
function snow_course_comp_lineplot(;era_load_func, savedir,
    ...
    timepick = 4, basin_to_station = snow_course_from_basin,
    ...
    for basin in ERA.usable_basins
        ...
        eradata = [filter(x -> (station_compare_args[:median_group_func])(x.datetime) == timepick, d) for d in eradata]
