...
for eratype in ERA.eratypes
    ...
    datavec = seasonal_datagen(;eratype, stat_name = :fom_rmsd,
    load_era_func = load_plain_nn, times_to_select = mymonths)
    [data[isnan.(data)].=0 for data in datavec]
    style_kwargs = (;title="ERA5 $eratype vs SNOTEL by Month",
    ylabel = "Fraction of Median RMSD", xlabel = "year", margin = 5Plots.mm)
    cvec = [:yellow, :orange, :red, :purple, :blue, :lightseagreen, :green]
    labels = string.(mymonths)
    plotname = "$eratype basin summary.png"
    error_bar_plot(datavec, dir; cvec, labels, plotname, style_kwargs,
