...
dir = "../vis/plain_nn/seasonal_rmsd"
...
for (climo, climoname) in zip(("", "climo_"), ("", "Climatology"))
    for (stat, statname, bounds) in zip(("diff_mean", "rmsd", "bias_corrected_rmsd"), ("Bias", "RMSD", "Bias Corrected RMSD"), ((-0.2,0.2),(0, 1), (0,1)))
        for eratype in ERA.eratypes
            ...
            datavec = seasonal_datagen(;
                eratype,
                stat_name = "$(climo)fom_$stat",
                load_era_func = load_plain_nn,
                times_to_select = mymonths,
                basins = snotel_basins
            )
            [data[isnan.(data)] .= 0 for data in datavec]
            title = if isempty(climo) "ERA5 $eratype vs SNOTEL by Month" else "Climatology vs SNOTEL by Month" end
            style_kwargs = (;
                title,
            ...
            plotname = "$eratype $statname $climoname basin summary.png"
            error_bar_plot(
                datavec,
                dir;
                cvec,
