...
for basin in ERA.basin_names
    ...
    for eratype in ERA.eratypes
        basin_to_fline = jldopen(joinpath(ERA.NOHRSCDATA, "$(eratype)_basin_to_flines.jld2"))["basin_to_flines"]
        ...
        for id in flines
            ...
            analyzed_data = comparison_summary(data, [:mean_era_swe, :gamma], :datetime; anom_stat="median").monthlymeandata
            push!(used_flines, id)
           # basin == "Chena" && display(analyzed_data)
            push!(nohrsc_data, analyzed_data)
        end

        basinmean = sort!(basin_aggregate(nohrsc_data, used_flines; timecol=:month), :month)
