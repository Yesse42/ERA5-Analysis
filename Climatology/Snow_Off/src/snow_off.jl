...
for (eratype, erafile) in zip(ERA.eratypes, ERA.erafiles)
    ...
    for basin in ERA.basin_names
        ...
        basin_mean_snow_off = vec(
            mapslices(
                x -> mean(map(Float32, filter(x -> !isa(x, SnowOffType), x))),
                basinwide_data;
                dims = 2,
            ),
        )
    ...
    minday, maxday = minimum(minimum.(ERA.skipnaormiss.(basin_snowoff))),
    maximum(maximum.(ERA.skipnaormiss.(basin_snowoff)))
    daysofyear = (minday - 2):5:(maxday + 2)
    daylabels =
        Dates.format.(Date(2021) .+ Day.(round.(daysofyear) .- 1), dateformat"mm/dd")

    myp = plot(;
        title = "ERA5 $eratype Basin Mean Snow Off Dates",
        xlabel = "year",
        ylabel = "Mean date of snow off",
