...
function single_year_single_point_snow_off(sd, dates; min_snow)
    if ismissing(sd[begin]) return NaN end
    ...
    if current_idx ≡ nothing return NaN else return dayofyear(dates[current_idx]) end
end
...
function year_snow_off(sd, times; min_snowy_days_before_snowpack_established)
    ...
    for (t_idx,this_year) in enumerate(minyear:1:maxyear)
        ...
        if current_idx+ndays-1 > max_idx
            daysofyear = Date(this_year):Day(1):Date(this_year)+Day(max_idx-current_idx)
            ndays = length(daysofyear)
        end
        this_year_sds = @view sd[:,:, current_idx:current_idx+ndays-1]
        for i in 1:size(sd,1), j in 1:size(sd,2)
            snowoff[i,j,t_idx] = single_year_single_point_snow_off(@view(this_year_sds[i,j,:]), daysofyear; min_snow=min_snowy_days_before_snowpack_established)
        end
        current_idx += ndays
    end
    return snowoff
...
for (eratype, erafile) in zip(ERA.eratypes, ERA.erafiles)
    ...
    ax = plt.subplot(1,1,1,projection = ccrs.Gnomonic(central_longitude=-147, central_latitude=65))
    #ccrs.AlbersEqualArea(central_longitude = -150,standard_parallels = (57,69))
    fig.add_axes(ax)

    cfplot = ax.contourf(longrid, latgrid, snow_off_mean, transform=ccrs.PlateCarree(), levels = 20, cmap = "cividis_r")
    cont = ax.contour(longrid, latgrid, snow_off_mean, transform=ccrs.PlateCarree(), levels = 10, colors="pink", linewidths=0.5)

    fmt(value) = "$(round(value))"

    ax.clabel(cont, cont.levels, inline=true, fmt=fmt, colors="black", fontsize=6)

    plt.colorbar(cfplot, shrink = 0.7, ticks = 0:15:360, label="Mean Day of year at which \"snow is off\"")

    ax.set_extent(ERA.ak_bounds, crs=ccrs.PlateCarree())
    ax.gridlines()
    ax.coastlines()
    ax.set_title("ERA5-$eratype Mean (1979-2021) Snow off date, snowpack considered established after 30 days", fontsize = 12)
