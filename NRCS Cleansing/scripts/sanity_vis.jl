...
include.(
    joinpath.(ERA.COMPAREDIR, "Load Scripts", ("load_snow_course.jl", "load_snotel.jl"))
)

for (network, loadfunc, label) in
    zip(ERA.networktypes, (load_snotel, load_snow_course), (:snotel_swe, :snow_course_swe))
    basin_to_stations =
        jldopen("../data/cleansed/$(network)_basin_to_id.jld2")["basin_to_id"]
