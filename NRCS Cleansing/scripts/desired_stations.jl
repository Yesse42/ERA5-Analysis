...
huc_to_basin = Dict([
    huc => basin for (hucs, basin) in zip(ERA.allowed_hucs, ERA.basin_names) for huc in hucs
])

for network in ERA.networktypes
    basin_to_id = Dictionary{String, Vector{String}}(
        ERA.basin_names,
        [String[] for i in 1:length(ERA.basin_names)],
    )
