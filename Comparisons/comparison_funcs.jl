...
function compare_with_ERA(;station_path, station_meta_path, dailyname::Symbol, monthlyname::Symbol, id_colname=:ID, savedir = "../data")
    ...
    for (i,(station, data)) in enumerate(zip(keys(data_by_station), data_by_station))
        for (j,eratype) in enumerate(eratypes)
            ...
            if isempty(withmonth) analysisarray[i,j]=missing; dailydata[i,j] = missing; continue end
    ...
    jldsave("$savedir/$monthlyname.jld2", monthlyname = labeled_data_array)
end