...
for (eratype, erafile) in zip(ERA.eratypes, ERA.erafiles)
    ...
    for fline in flines
        ...
        lims = (
            minimum(minimum.((gamma_swe, mean_era_swe))) - 1,
            maximum(maximum.((gamma_swe, mean_era_swe))) + 1,
        )

        myplot = scatter(
            gamma_swe,
            mean_era_swe;
            title = "NOHRSC vs. ERA5 $eratype SWE Scatter, Flight Line: $fline",
            xlabel = "NOHRSC SWE (in.)",
            ylabel = "ERA5 SWE (in.)",
            label = "",
            aspect_ratio = :equal,
            xlims = lims,
        ...
        annotations =
            tuple.(gamma_swe, mean_era_swe, year.(fline_measurements.date[valid_time_idxs]))
