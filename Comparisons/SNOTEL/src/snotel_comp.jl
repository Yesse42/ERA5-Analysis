...
stations = CSV.read(joinpath(nrcsdatadir, "SNOTEL_Metadata.csv"), DataFrame)
...
labeled_data_array = AxisArray(analysisarray, station = string.(collect(keys(data_by_station))), eratype = eratypes)
...
using JLD2
display(labeled_data_array)
jldsave("../data/snotel_monthly_data.jld2", snotel_monthly_data = labeled_data_array)