...
basin_to_snow_course = jldopen(joinpath(ERA.NRCSDATA, "cleansed", "Snow_Course_basin_to_id.jld2"))["basin_to_id"]
...
for (eratype, erafile) in zip(ERA.eratypes, ERA.erafiles)
    ...
    for basin in ERA.basin_names
        ...
        for poly in polys
            ...
            poly_plot = mpatches.Polygon(poly, closed=true, alpha=0.5, facecolor = "grey", edgecolor = "salmon", linewidth = 2.,
        ...
        era_lonids, era_latids = [findall(between(tup), arr) for (tup, arr) in zip((lonbounds, latbounds),(lon,lat))]
        ...
        ax.scatter(longrid[plotmask][:], latgrid[plotmask][:], s=0.125, transform = ccrs.PlateCarree(), color="fuchsia")
        ...
        pcm = ax.contourf(longrid[plotmask], latgrid[plotmask], elev[plotmask], norm=divnorm, 
            cmap=terrain_map, shading="auto", transform = ccrs.PlateCarree(), zorder = 0)
        ...
        for (id_dict, mycolor) in zip(stationtype_ids, mycolors)
            ...
            for id in ids
                ...
                era_id_idx = findfirst(==(id), era_chosen_points.stat_id)
                ...
                arr(x) = [x]
                ax.scatter(arr.(stat_point)...; transform = ccrs.PlateCarree(), color = mycolor, s=30)
                arrow_offset = (lon[era_id_data.lonidx], lat[era_id_data.latidx]) .- stat_point
                ax.scatter(arr.(arrow_offset .+ stat_point)...; transform = ccrs.PlateCarree(), color = "orange", s=20)
                #Draw arrow to ERA point
                ax.arrow(stat_point..., arrow_offset...; transform = ccrs.PlateCarree(), linewidth = 1)
            end
        end
