...
for (era_type, era_file) in zip(ERA.eratypes, ERA.erafiles)
    geopotential = NCDataset("../data/unaligned_geopotentials/$(era_type)_geopotentials.nc", "r")
    ...
    filename = "../data/$(era_type)_aligned_elevations.nc"
    if isfile(filename) rm(filename) end
    ds = Dataset(filename, "c")
    defDim.(Ref(ds), ("longitude","latitude"), length.((outlon, outlat)))
    defVar.(Ref(ds), ("longitude","latitude"), (outlon, outlat), tuple.(("longitude","latitude")))
