...
geofiles = ["base","land"].*"_geopotentials.nc"
...
#And now plot the indices and the elevation at each station of interest, as well as the actual grid point
pyplot()
...
for (i, name) in enumerate(["Base", "Land"])
    ...
    for (row, nnid, geonnid) in zip(eachrow(stations), nearest_neighbor_idxs, geo_neighbor_idxs)
        ...
        idarr = CartesianIndices(glacierarrs[i][:,:,1])[nnid[i]-offset_index:nnid[i]+offset_index]
        ...
        #Now examine the 4 "closest" points; take the first one that isn't glacier
        outidx = nothing
    ...
    CSV.write(name*"_nearby_point_idx.csv", out_index_df)
end
