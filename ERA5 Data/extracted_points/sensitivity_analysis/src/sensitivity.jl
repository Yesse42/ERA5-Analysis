...
snow_course_stations =
    CSV.read(joinpath(ERA.NRCSDATA, "cleansed", "Snow_Course_Metadata.csv"), DataFrame)
era_datas =
    Dictionary(ERA.eratypes, Dataset.(joinpath.(ERA.ERA5DATA, ERA.eratypes, ERA.erafiles)))
...
function sensitivity(eratype, erafile, basin, offset, weight_func)
    ...
    function era5_from_id(_, _, id)
        ...
        era_df =
            DataFrame(; datetime = time, era_swe = @view(sd[era_idx, :]), copycols = false)
        return era_df
    end
...
for basin in ERA.basin_names
    for (eratype, erafile) in zip(ERA.eratypes, ERA.erafiles)
        ...
        best_idx = Tuple(
            first(best_index for best_index in best_idxs if nmiss_arr[best_index] == 0),
        )
        ...
        insert!(
            ...
            (basin, eratype),
            (offset = offsets[best_idx[1]], weight = relweights[best_idx[2]]),
        )
        ...
        p = plot(
            ...
            p2;
            layout = (2, 1),
            plot_title = "ERA5-$eratype S. Course in $basin Basin",
        )
