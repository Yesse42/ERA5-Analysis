...
datasets =
    NCDataset.(
        [
            "ERA5 Data/Base/ERA5-SD-1979-2021-DL-2022-6-15.nc",
            "ERA5 Data/Base/ERA5-SD-2022-2022-DL-2022-6-16.nc",
        ],
        "r",
    )
...
for vname in ("longitude", "latitude")
    defVar(
        combo,
        vname,
        datasets[1][vname].var[:],
        (vname,);
        attrib = datasets[1][vname].attrib,
    )
end
...
for (k, v) in datasets[1]["sd"].attrib
    if k in ("_FillValue", "missing_value", "add_offset", "scale_factor")
        continue
    end
    sdvar.attrib[k] = v
end
