...
for eratype in ERA.eratypes
    ...
    nn_points = best_points(;
        ...
        elevationdata,
        metadatas,
        network_metrics = Dict(ERA.networktypes .=> plain_distance),
        searchwindow = windowsize,
    )
    CSV.write(
        "$savedir/$(eratype)_best_ids.csv",
        select(nn_points, :id, :best => collect => [:lonidx, :latidx]),
    )
    for row in eachrow(nn_points)
        ...
        CSV.write(
            joinpath(savedir, "$(eratype)/$(row.id).csv"),
            DataFrame(; sd = sd_at_loc),
        )
    end
