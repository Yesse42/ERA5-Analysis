...
for eratype in ERA.eratypes
    ...
    for (fold_type, fold_func) in zip(fold_types, fold_funcs)
        ...
        for fold_num in 1:n_folds
            ...
            network_metrics = Dictionary(ERA.networktypes, [custommetric(tfilter, (x...)->pom_rmsd(x...;median_groupfunc = (y->1))) for tfilter in network_timefilters])
        ...
        out_dict = Dictionary{String, NamedTuple{(:time, :sd), Tuple{Vector{Date}, Vector{Float32}}}}()
        out_locs = Dictionary{String, NamedTuple{(:foldnum, :lonlat), Tuple{Vector{Int}, Vector{NTuple{2, Int}}}}}()
        for (i, station) in enumerate(foldarr[1].id)
            ...
            for (foldnumber, (df, timefunc)) in enumerate(zip(foldarr, foldtimefuncs))
                ...
                allowed_era_indices = [i for (i,time) in enumerate(eratime) if is_allowed(time)]
                append!.(Tuple(outdata), (eratime[allowed_era_indices], sd[df.best[i]..., allowed_era_indices]))
