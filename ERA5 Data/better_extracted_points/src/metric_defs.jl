...
custommetric(timefilterfunc, deviationfunc) = function metric(;
    eratype,
    stationmetadata,
    glacierbool,
    eralonlat,
...
function pom_rmsd(time, era, stat; median_groupfunc)
    ...
    pom(colname) = function f(val, time)
        idx = findfirst(==(median_groupfunc(time)), medians.groupcol)
        return val / medians[idx, colname]
    end
...
coursemetric =
    custommetric(endmarch_beginapril, (x...) -> pom_rmsd(x...; median_groupfunc = (x -> 1)))
