...
for eratype in ERA.eratypes
    for basin in ERA.basin_names
        ...
        rmsd_data = rmsd_data[
            mapslices(row -> all((!isnan).(row[2:end])), Array(rmsd_data); dims = 2)[:],
            :,
        ]
...
jldsave(
    "../../elevation_weight_data/optimal_relative_weights.jld2";
    optimal_relative_weights = optimal_search_weights,
)
