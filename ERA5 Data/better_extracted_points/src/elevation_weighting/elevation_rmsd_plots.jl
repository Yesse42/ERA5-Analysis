...
function theil_sen(x, y)
    length(x) ≠ length(y) && throw(ArgumentError("x and y are different lengths; bad bad bad"))
...
for basin in ERA.basin_names
    ...
    for (i, eratype) in enumerate(ERA.eratypes), (j, (diffstat, statname)) in enumerate(zip((:eldiff, :dist), ("Absolute Elevation Difference", "Great Circle Distance")))
        ...
        plot!(plotmat[i,j], [0, maximum(diffs)], [theil_sen_coefffs[1],theil_sen_coefffs[1] + theil_sen_coefffs[2]*maximum(diffs)], label = "", linewidth = 2)
    end
